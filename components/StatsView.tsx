...
import { HistoryRecord, WordStatsMap, Article, SavedItem } from '../types';
import { ChartBarIcon, TrashIcon, CheckCircleIcon, BookOpenIcon, EyeIcon, CardIcon } from './Icons';
...
export const StatsView: React.FC<StatsViewProps> = ({ 
  ...
  const sortedWords = Object.entries(wordStats)
    .map(([word, data]) => {
        // Calculate frequency relevant to the current article if needed
        const articleFrequency = currentArticleId 
            ? data.occurrences.filter(o => o.articleId === currentArticleId).length 
            : 0;
        return {
            word,
            ...data,
        ...
        <div className="max-w-4xl mx-auto pb-20">
            ...
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-slate-500 text-sm">
                        {currentArticleId 
                            ? `Stats Context: ${getArticleTitle(currentArticleId)}` 
                            : "Global Statistics"
                        }
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                <div className="bg-white p-5 rounded-xl border border-indigo-100 shadow-sm">
                    <p className="text-3xl font-bold text-slate-800">{Object.keys(wordStats).length}</p>
                    <p className="text-sm text-slate-500">Unique Words Met</p>
                </div>
                ...
                <div className="bg-white p-5 rounded-xl border border-amber-100 shadow-sm">
                    <p className="text-3xl font-bold text-slate-800">{totalLookups}</p>
            ...
            <div className="flex gap-4 border-b border-slate-200 mb-6">
                <button 
                    ...
                    className={`pb-3 px-2 text-sm font-bold transition-colors border-b-2 ${activeTab === 'history' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                    Activity Log
                </button>
                <button 
                    onClick={() => setActiveTab('frequency')}
                    className={`pb-3 px-2 text-sm font-bold transition-colors border-b-2 ${activeTab === 'frequency' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                    Word Frequency
                </button>
            </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-semibold">
                            <tr>
                                ...
                                <th className="px-6 py-3">In This Article</th>
                            ...
                            {sortedWords.map((data) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                             <span className={`text-sm font-mono font-bold ${data.articleFrequency > 0 ? 'text-emerald-600' : 'text-slate-300'}`}>
                                                 {data.articleFrequency}
                                             </span>
                                        </td>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="divide-y divide-slate-100">
                        {history.length === 0 ? (
                            ...
                            history.map((record) => {
                                // Find current article stats if available
                                const cleanWord = record.original.toLowerCase();
                                const stats = wordStats[cleanWord];
                                const currentArticleFreq = currentArticleId && stats 
                                    ? stats.occurrences.filter(o => o.articleId === currentArticleId).length 
                                    : 0;
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-0.5">
                                            ...
                                            {/* Show Frequency context if relevant */}
                                            {currentArticleFreq > 0 && record.type === 'word' && (
                                                <span className="text-xs font-medium bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded">
                                                    Seen {currentArticleFreq}x in current article
                                                </span>
                                            )}
                                        </div>
